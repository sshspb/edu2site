...
function rwStewards(callback) {
  ...
  // Из таблиц UNI и OTWU вытаскиваем stewards 
  ...
  // логин и пароль пользователя сайта buhedu в таблице OTWU
  Firebird.attach(config_local.firebird, function(err, fdb) {
    if (err) throw new Error('ошибка  Firebird.attach');
      
    fdb.query("SELECT LOGIN, PASWD, ROLE, NAME FROM OTWU WHERE LOGIN<>'' AND PASWD<>''", 
      function(err, otw) {
        if (err) throw new Error('ошибка  fdb.query');
        for (var i = 0; i < otw.length; i++) {
          otw[i].otwName = otw[i].NAME.trim().replace(/^(?:ЦФ )?(\S+)(?:\s+)(.\.)(?:\s*)(.\.).*/, "$1 $2$3");
          // console.log('1<<<', i, '><', otw[i].otwName, '><', otw[i].NAME.trim(), '>');
        }

        // строим stewards - вытащим из uni список ответственных
        // а login/password возьмём из otwu, убирая повторения 
        uni.sort(function(a,b) { return a.stewardName.localeCompare(b.stewardName) });
        otw.sort(function(a,b) { return a.otwName.localeCompare(b.otwName) });
        stewards = [], stewardName = null;
        var jChief = 0, jUni = 0, jOtw = 0, lastName = "@@@", lastUser = "@@@", stewardId = null;
        while (jUni < uni.length) {

            while ((jUni < uni.length) && 
                  ((jOtw == otw.length) || (otw[jOtw].otwName.localeCompare(uni[jUni].stewardName) > 0))) {
                if (lastName != uni[jUni].stewardName) {
                  ...
                  while ((jChief < chiefs.length) && (chiefs[jChief].chief.localeCompare(uni[jUni].stewardName) < 0)) {
                    jChief++
                  }
                  if ((jChief < chiefs.length) && (chiefs[jChief].chief.localeCompare(uni[jUni].stewardName) == 0)) {
                    chiefs[jChief].stewardId = stewardId;
                  }
                    // console.log('2<<<', jUni, '><', uni[jUni].stewardName);
                }
                uni[jUni]._steward = stewardId;
                jUni++
            }
            
            while ((jOtw < otw.length) && 
                  ((jUni == uni.length) || (otw[jOtw].otwName.localeCompare(uni[jUni].stewardName) <= 0))) {
              // уникальность - у ТБ у одного работника несколько login
              if (lastUser != otw[jOtw].otwName) {
                lastUser = otw[jOtw].otwName
                if (jUni < uni.length) lastName = uni[jUni].stewardName;
                stewardId =  new ObjectId();
                stewards.push({
                  "_id": stewardId,
                  "name": otw[jOtw].otwName,
                  "role": null,
                  "login": null,
                  "salt": null,
                  "hashedPassword": null
                });
                var jStw = stewards.length - 1;
                var password = otw[jOtw].PASWD.trim();
                var salt = Math.random() + '';
                stewards[jStw].login = otw[jOtw].LOGIN;
                stewards[jStw].salt = salt;
                stewards[jStw].hashedPassword = crypto.createHmac('sha1', salt).update(password).digest('hex');
                if (otw[jOtw].ROLE.trim()=='R') {
                  stewards[jStw].role = 'booker';
                } else {
                  stewards[jStw].role = 'master';
                }
                while ((jChief < chiefs.length) && (chiefs[jChief].chief.localeCompare(uni[jUni].stewardName) < 0)) {
                  jChief++
                }
              ...
              // console.log('3<<<', jStw, '><', stewards[jStw].name, '><', stewards[jStw].login, '><', stewards[jStw].role);
              }
              jOtw ++;
            }
          
          if (jUni < uni.length) uni[jUni]._steward = stewardId;
          jUni++
        }
...
//          stewards[i].nameTrans = stewards[i].name.replace(/\s/g, '');
//        }
        fdb.detach();
      });
